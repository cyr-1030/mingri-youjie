...
function buildPlan(input) {
  ...
  const accessoryItem = itemFor(own, '配饰', direction.accessory, conditions, reason, direction.palette)
  ...
  const makeup = input.occasion === '居家' ? makeupBase : `${makeupBase}；${scene.makeup}`
  ...
  const styling = `${baseStyling}。${scene.advice}`
  ...
  return {
    ...
    mood: `${style.mood}${secondary ? `，加入一点${secondary.name}的感觉` : ''}；${focus}`,
    items, photoCount, makeup, hair, reminder: fullReminder, substitute, focus,
